...
export const Navbar = () => {
    ...
    return (
        <>
        <nav className="flex items-center w-100 items-center bg-primary justify-between py-2 xl:px-24 lg:px-12   sm:p-4 2xl:px-32 sticky top-0 left-0 z-10">
            <Link  href="/">
                <a className="inline-flex"> 
                        <Image height='50px' width='160px' src="/logo.png" alt="robotdorm-logo" />
                </a>
            </Link>
            <div className= { `${active? 'flex' : 'hidden'} z-0 lg:flex 2xl:w-2/5 xl:w-1/2 lg:w-8/12 lg:justify-between lg:flex-row items-center lg:p-0 lg:h-auto lg:bg-transparent lg:relative lg:text-base text-white sm:absolute sm:flex-col sm:w-full sm:top-0 sm:left-0 sm:pt-20 sm:bg-primary sm:h-screen sm:self-start sm:flex-start sm:text-2xl sm:z-10`}   >
                ...
                <Link href="/Login"><a className="md:hidden lg:flex items-center bg-white text-primary px-6 py-2">Login /Sign up</a></Link>
            </div>
